...
kmean <- function(X, K, threshold=1e-6, max_iter=10){
  ...
  res <- structure(list(label=clustered, total_distance=tot_dis, cluster_centers=cent_cord), class= "KMEAN_res")
...
em <- function(ys, k, threshold=1e-9, max_iter=201){
  ...
  res <- structure(list(label=clustered, params=params), class= "EM-GMM_res")
  return(res)
}
...
emgmm_df <- data.frame(data=data, label=res$label)
plot(label~data, data=emgmm_df)
```
