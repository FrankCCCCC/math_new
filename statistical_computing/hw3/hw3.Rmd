...
em <- function(ys, k, threshold=1e-9, max_iter=201){
  ...
  res <- structure(list(data=ys, cluster=clustered, params=params, dens_table=dens_table), class= "EM-GMM_res")
  return(res)
}
...
for(i in 1:M){
  ...
  # weights
  ws_1[i] <- em_gmm_model$params[1, 1]
  ws_2[i] <- em_gmm_model$params[1, 2]
  
  # mus
  mu_1[i] <- em_gmm_model$params[2, 1]
  mu_2[i] <- em_gmm_model$params[2, 2]
}


#pram_mean <- matrix(c(mean(ws_1), mean(ws_2)))
#print("Est Mean")
#print(pram_mean)
```
