...
update_comm <- function(){
  comm$N_k <- colSums(r_nk) + 1e-10
}
```
...
```{r}
update_r_nk <- function(log_rho_nk){
...
update_log_r_nk <- function(log_r_nk){
...
```{r}
cal_elbo <- function(D, K, N_k, S_k, x_bar_k, r_nk, log_r_nk, log_pi, log_Lambda, alpha_0, alpha, beta_0, beta_k, nu_0, nu_k, W_0, W_0_inv, W_k, m_0, m_k){
...
```{r}
add_animate <- function(dt_all, dt, iter, m_k, W_k, beta_k, nu_k, alpha){
...
vb_gmm <- function(X, 
  ...
  if (is_animation) { # Create animation for initial assignments
    my_z = mixture_pdf_t(model = list(m = m_k, W = W_k, beta = beta_k, nu = nu_k, alpha = alpha), data = dt)
    dt_all <- rbind(dt_all, dt[, z := my_z] %>% .[, iter := 0])
  }
  ...
  for (i in 2:max_iter) {
    ...
    ##-------------------------------
    for (k in 1:K) {
      log_rho_nk[, k] <- update_log_rho_nk(X, k, m_k, log_pi, log_Lambda, D, nu_k, W_k)
    }
    # Responsibilities using the logSumExp for numerical stability
    res <- update_r_nk_both(log_rho_nk)
    log_r_nk <- res$log_r_nk
    r_nk <- res$r_nk
    
    ##-------------------------------
    # Variational M-Step
    ##-------------------------------
    N_k <- update_N_k(r_nk, epsilon)
    for (k in 1:K) {
      x_bar_k[, k] <- update_x_bar_k(k, r_nk, X, N_k)
      S_k[, , k] <- update_S_k(k, X, x_bar_k, r_nk, N_k)
    }
    # Update Dirichlet parameter
    alpha <- update_alpha(alpha_0, N_k)
    # Compute expected value of mixing proportions
    pi_k <- update_pi_k(alpha_0, N_k, K, N)
    
    # Update parameters for Gaussia-nWishart distribution
    beta_k <- update_beta_k(beta_0, N_k)
    nu_k <- update_nu_k(nu_0, N_k)
    for (k in 1:K) {
      # 10.61
      m_k[, k] <- update_m_k(k, beta_k, m_0, N_k, x_bar_k)
      # 10.62
      W_k[, , k] <- update_W_k(k, W_0_inv, N_k, S_k, beta_0, x_bar_k, m_0)
    }
    # Update expectations over \pi and \Lambda
    # 10.66
    log_pi <- update_log_pi(alpha)
    for (k in 1:K) { # 10.65                                              
      log_Lambda[k] <- update_log_lambda_k(k, nu_k, D, W_k)
    }
    
    ##-------------------------------
    # Variational lower bound
    ##-------------------------------
    L[i] <- cal_elbo(D, K, N_k, S_k, x_bar_k, r_nk, log_r_nk, log_pi, log_Lambda, alpha_0, alpha, beta_0, beta_k, nu_0, nu_k, W_0, W_0_inv, W_k, m_0, m_k)
