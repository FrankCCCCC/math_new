...
vb_gmm <- function(X, 
  ...
  obj <- structure(list(X = X, K = K, N = N, D = D, pi_k = GM$pi_k, 
...
mus <- gen_mus(K, D, scale)
sigmas <- gen_sigmas(K, D, df=df)
coef <- rep(1, K)
...
ggplot() + 
  geom_point(data = data.frame(X), mapping = aes(X, Y)) + 
...
p <- ggplot() + geom_point(data = data.frame(X), aes(X, Y)) + 
