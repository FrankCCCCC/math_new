...
cal_tot_dist <- function(X, cent_cord){
  n <- length(X)
  d <- 1
  K <- length(cent_cord[,1])
  
  X <- data.frame(X)
  dis_table <- data.frame(matrix(0, nrow=n, ncol=K))
  within_dis <- data.frame(matrix(0, nrow=n, ncol=K))
  clustered <- array(0, c(n))
  
  for(k in 1:K){
    dis_table[, k] <- apply(X, 1, dis, cent_cord[k, ])
  }
  ...
  for(k in 1:K){
    ...
    within_dis <- sapply(X[idxs, ], dis, cent_cord[k, ])
  ...
  tot_dis <- sum(dis_table)
  
  #idxs <- which(clustered == 1)
  #cbind_sel <- data.frame(X)[idxs, ]
  #print(idxs)
  #print(cbind_sel)
...
vb_gmm_mus <- unique(as.data.frame(t(vb_gmm_model$m)))
kmean_mus <- kmean_model$centers

#head(kmean_mus)
#head(X)
#cal_tot_dist(X, kmean_mus)
#cal_tot_dist(X, vb_gmm_mus)
```
...
```{r}
#draw_contour(X, vb_gmm_model, aes(eruptions, waiting))
#draw_contour(X, vb_gmm_model, aes(eruptions, waiting))
...
clusplot(X, kmean_model$cluster, main='2D representation of the Cluster solution',
         color=TRUE, shade=TRUE,
         labels=2, lines=0)

clusplot(X, vb_gmm_model$cluster, main='2D representation of the Cluster solution',
         color=TRUE, shade=TRUE,
         labels=2, lines=0)
```